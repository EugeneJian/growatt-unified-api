  ...
  - Quick Site Setup
    ...
    - EMS
      - Work Mode
        - TOU Mode
          - Mode Settings
            - Time Period1（hold3038-3039）
            - Add Time Period （hold3040-3045）
        - Backup Mode
          - Mode Settings
        - Peak Shaving Mode
          - Mode Settings（hold3306）
      ...
      - Battery Setting
        ...
        - Discharge Stop SOC
          - 并网放电停止SOC（hold3067）
          - 离网放电停止SOC（hold3037）
        - Battery Charging From Grid（hold3049） / 开关按钮
        - Max.Charge Power From Grid（hold3311）
        - Charge Stop SOC from Grid（hold3312）
      - Off-Grid Enable（hold3079） / 开关按钮
      - AC-Couple(hold612) / 开关按钮
    ...
    - Parallel Setting / 开关按钮 / XH/HU 机型都不展示
      ...
      - COM Address（hold30）
        - 1- 254
    - Auto test (only for Italy)（hold4371） / 根据意大利机型和功率选择显示（<=11kW）
  - 直连设置（shinetools)平台
    - Network Configuration and General Setting / 讨论是否简化为 General Setting
      - Network Configuration
      - Active Power
        - Active Power %（hold3）
        - Memory Enable（hold2） / 开关按钮（先设置记忆开关，再设置有功功率，记忆才生效）
      - Off-Grid Paramater Setting 改成Off-Grid Setting
        - SYN Box Enable （hold3020） / 开关按钮 / XH有，HU没有
        - Off-Grid Enable（hold3079） / 开关按钮
        - Off-Grid Frequncy（hold3081）
          - 50Hz
          - 60Hz
        - Off-grid Voltage（hold3080）
          - 230V
          - 208V
          - 240V
          - 220V
      - Work Mode(改成 Retrofit/Parallel )（hold30118）
        - Default mode
        - System Retrofit
        - Multiple machines in parallel
      - COM Address（hold30）
        - 1- 254
      - Reset（hold33） / 研发确认有没有用 / 选项用于二次确认，后面建议只保留recover，加一个二次确认的弹窗“请确认是否恢复出厂设置”
        - Doesn't recover
        - recover
      - Dry Contact Control
        - Dry contact enable（hold662） / 开关按钮
        - Status of Dry Contact（input3119）
        - Dry Contact Mode（hold662）
          - Power(Inverter/Surplus)
          - Time
          - Time Power
        - Power(Inverter/Surplus)
          - ON
            - Feed in/production power （hold3017）
            - Start-up delay time（hold664）
          - OFF
            - Feed in/production power
            - Min.Duration（hold665）
      - Inverter Time（hold45-50）
      - Language（hold15）
      - Inverter Power OFF/ON（hold0） / 开关按钮
    ...
    - Grid Parameter Setting
      ...
      - PF Setting(hold2001)
        ...
        - Inductive Reactive Power%
          - Inductive Reactive Power %(hold4)
          - Memory Enable(hold2) / 开关按钮
        - Capactive Reactive Power%
          - Capactive Reactive Power %(hold4)
          - Memory Enable(hold2) / 开关按钮
        - Inductive PF/Capacitive PF
          - Inductive PF/Capacitive PF(hold5)
          - Memory Enable(hold2) / 开关按钮
        - PF Curve In/Out Vac(hold628-631)
          - PF Curve In Vac
          - PF Curve Out Vac
        - Custom PF curve
          - PF Curve Setting 1
      ...
      - Safety Parameters / 需要研发对其相同设置项，只保留一个
        ...
        - Grid Connection Restriction / 漏了Vpv start (中文描述把PV和AC电压都叫并网电压，需修改描述)
          - 并网电压高点（hold64）
          - 并网电压低点（hold65）
        - Load Drop Rate（hold633）
        - 10 Minutes Protection（hold634）
        ...
        - Third-stage Voltage&Frequency
          ...
          - 欠频（hold62）
          - 过频（hold63）
        - P(U)Function（hold641） / 研发确认细节 / 确认OK，不改动
        - Voltage Ride Through（hold636-637）
        - Overfrequency Derating（hold638）
        - Underfrequency Increasing（hold334）
        - ROCOF（hold642）
        - DCI（hold632）
        - Other(?)
        - Active Power Control（hold3）
        - Reactive power mode（hold89）
        - Q(U)Mode（hold159）
      - Wide Operating Voltage Range（hold236）
        - 0
        - 1
        - 2
      - Enale N Linec（hold232） / 开关按钮
      - RRCR Enable（hold640） / 开关按钮 / 确认是否是 DRMS 和 RRCR 选择 / DRMS是澳洲安规显示，RRCR是欧洲安规，本质上是一样的，建议和安规绑定显示DRMS或RRCR
      - PU Enable 删除
    ...
    - Device Information / XH漏了离网盒参数，HU不需要离网盒参数
      - PV Voltage/Current（input3-17）
      - AC Volt/Current/Power/Freq（input37-48）
      - String Volt/Current / XH/HU没有，应删除
      - 离网盒参数 / 原本就有
        - 离网盒子使能（hold3020）
        - 其他参数（input3279-3324）
      - Off-grid Parameter
    ...
    - Advanced Setting
      - AFCI
        - Enable AFCI（hold541） / 开关按钮
        - AFCI Self-Test（hold542） / 开关按钮
        - AFCI Reset（hold543） / 开关按钮
        - AFCI Threshold1（hold544）
        - AFCI Threshold2（hold545）
        - AFCI Threshold3（hold546）
        - Max Accumulated Counts for Over FFT Value（hold547）
        - AFCI Curve Scan（hold548）
      - PV Input Mode（hold399）
        - Independent MPPT Mode
        - DC Source Mode
        - Parallel MPPT Mode
      - Smart Diagnosis
        - Smart I-V Curve Sc（hold250） / I-V Scan for each MPPTan
        - Abnormal Waveform Record（hold259） / Quickly locate problem remotely via waveform record.
        - Real-time Waveform Record（hold260） / Mointoring inverter voltage and current quality,etc.
        - One-Click Diagnosis（hold265-266） / Including l-V curve,AC waveform record,THDV and grid cable impedance.
      - Anti-islanding Protection（hold230） / 开关按钮
      - Basic Parameters / 研发确认是否删除
        - Baud Rate Selection / 确认是否删除 / 设备升级后如果485口没有恢复波特率就需要用这个进行修改，可以通过寄存器设置直接修改，XH认为此处可删除
          - 9600bps
          - 38400bps
          - 115200bps
        - Clear History（hold32）
        - Model Setting（hold118-121）
        - PV Voltage 删除
