...
| data.vac1                          | double | 234            | 相电压 1，单位: V                                          |
| data.vac2                          | double | 233            | 相电压 2，单位: V                                          |
| data.vac3                          | double | 234.5          | 相电压 3，单位: V                                          |
...
| data                              | object | -              | 主数据对象                                                 |
| data.activePower                  | double | 0              | 电网取电功率（正值），单位: W                              |
| data.pac                          | double | 2871.4         | 交流输出功率，单位: W                                      |
| data.ppv                          | double | 3045.3         | PV发电功率，单位: W                                        |
| data.payLoadPower                 | double | 258.4          | 总负载功率（计算值），单位: W                              |
| data.reverActivePower             | double | 2781.9         | 馈入电网功率，单位: W                                      |
| data.batteryStatus                | int    | 3              | 电池总体状态                                               |
| data.batPower                     | double | 200.5          | 电池总充/放电功率（正值=充电，负值=放电，0=空闲），单位: W |
| data.priority                     | int    | 2              | 工作优先级                                                 |
| data.deviceSn                     | string | TEST123456     | 设备序列号                                                 |
...
| 参数名                              | 参数说明                                                     | 参数值说明                                                   |
| ----------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| time_slot_charge_discharge          | 分时段充放电 percentage值范围[-100,100] percentage>0充电 percentage<0放电 startTime：开始时间(utc) endTime：结束时间(utc) | 设置时间段(json格式:[{percentage:功率,startTime:开始时间(utc),endTime(utc):结束时间}],时间范围：[00:00,24:00], eg: {     "deviceSn": "DLP0DA7888",     "value": [{"startTime":"00:00","endTime":"23:59", "percentage":100 }],     "requestId":"12345678901234567890123456789012",     "setType":"time_slot_charge_discharge" } |
| duration_and_power_charge_discharge | 充放电时长和功率百分比 percentage值范围[0,100] 分为4种模式 1.selfConsumptionCommand(自发自用模式) 2.chargeOnlySelfConsumptionCommand(自发自用只充电模式) 3.chargeCommand(充电模式) 4.dischargeCommand(放电模式) 当设置成功后,设备将按照设定的模式运行,且时间 | 注:percentage仅chargeCommand,dischargeCommand必须 {     "deviceSn": "TEST123456",     "value": {         "duration": 10,         "percentage": 20,         "type": "dischargeCommand"     },     "requestId":"12345678901234567890123456789012",     "setType": "duration_and_power_charge_discharge" } |
| anti_backflow                       | 防逆流设置 antiBackflowEnabled：启用防逆流 percentage: 防逆流百分比 值范围[-100,100]，正值：逆流控制，负值：顺流控制 | {     "deviceSn": "HCQSKJMSJ1",     "value": {"antiBackflowEnabled":1,"percentage":20},     "requestId":"12345678901234567890123456789012",     "setType":"anti_backflow" } |