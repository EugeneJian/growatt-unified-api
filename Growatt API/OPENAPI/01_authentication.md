...
- Device onboarding typically starts from `bindDevice` with a known `deviceSn`; use object entries containing `deviceSn`, and add `pinCode` when the environment or the target device requires it.
...
| Bind device `bindDevice` | Supported; use object entries containing the `deviceSn` returned by `getDeviceList`, and add `pinCode` when required | Supported; use object entries containing `deviceSn`, and add `pinCode` when required |
