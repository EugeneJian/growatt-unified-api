...
本页根据 `docs/3 接口列表.md` 中的 `3.1`、`3.2` 与 `3.3` 章节整理认证模式与能力边界；若后续环境联调出现差异，应视为环境观察，不应覆盖本页基线口径。

## 基线确认的授权模式

| `grant_type` | 说明 | 基线明确给出的能力边界 |
| :--- | :--- | :--- |
...
- 厂商基线在两个 token 请求示例中都携带了 `redirect_uri`。
- `POST /oauth2/refresh` 的前提是持有 `refresh_token`；厂商基线没有再按 `grant_type` 对刷新能力做进一步拆分。
- 厂商基线中的 token 返回字段表统一列出了 `access_token`、`refresh_token`、`refresh_expires_in`、`token_type`、`expires_in`。
...
| 刷新 access token | 厂商基线提供 `POST /oauth2/refresh` | 厂商基线未额外拆分模式差异 |
...
- 如果需要环境联调经验，请阅读 [常见问题与排查 FAQ](./11_api_troubleshooting.md) 中的“联调观察（非基线规范）”部分。
